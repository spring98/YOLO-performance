...
해당 레포지토리는 전체코드가 아니며, 사용했던 모델 및 라이브러리의 타당성을 설명하기 위한 데모 입니다.
...
관련 코드는 server.py 파일로 업로드 되어있습니다.

<br/>

|  | 1번 | 2번 | 3번 | 4번 |
| --- | --- | --- | --- | --- |
| 평균 온도(도)| 42.20 | 38.36 | 38.68 | 29.23 |
| 평균 프레임(fps) | 3.65 | 11.55 | 4.76 | 5.21 |
...
3. 실험에 사용한 쿨러의 전력 소모를 추가적으로 고려할 때, 해당 쿨러는 5V, 1.2A 즉 6W 를 사용하므로 10분 (600초) 동안 사용하면 3600J 을 추가적으로 사용하게 됩니다.
   3번에 비해 4번은 458.90 J 에서 4385.92 J 로 9.55배 더 많은 전력을 사용합니다. 9.55배 더 많은 전력을 사용하여 평균온도 32% 감소, 평균프레임 9% 상승 효과를 얻을 수 있습니다.
...
3. 모델의 크기를 320 에서 640 으로 증가시켜 추론의 정확성을 높이고자 한다면 성능면에서 평균프레임이 2.42배 감소하고 에너지 효율면에서는 40.75배 나빠집니다.
4. 외부 쿨러를 사용하면 에너지 효율면에서는 9.55배 나빠지지만 평균온도는 32% 감소하며 성능면에서 프레임이 9% 상승합니다.
